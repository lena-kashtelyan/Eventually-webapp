...
public class PublicFBEventsWriter {
	...
	public void updateDB(JsonObject jsonResults) throws SQLException,
		...
		if (eventsArraySize > 40) {
			...
			try {
				...
				e.printStackTrace();
				System.out
						.println("ERROR: Problem when waiting for a thread to die.");
			}
		} else {
	...
	static void processEvent(JsonElement eventElement) {
		...
		if (!eventJSON.get("eventCoverPicture").toString().equals("null")) {
			eventProfilePicture = URLEncoder.encode(eventJSON
					.get("eventCoverPicture").toString().replace("\"", ""));
		} else if (!eventJSON.get("eventProfilePicture").equals("null")) {
			eventProfilePicture = URLEncoder.encode(eventJSON
					.get("eventProfilePicture").toString().replace("\"", ""));
		} else {
			eventProfilePicture = URLEncoder
					.encode("https://s-media-cache-ak0.pinimg.com/564x/f3/69/b4/f369b42357a27eb40068f675f62366ce.jpg");
		}
		...
		eventEndtimeString = eventEndtimeString.substring(4).replaceAll(
				"E[D,S]T", "");
		...
		int eventMaybeCount = Integer.parseInt(eventStats.get("maybeCount")
				.toString());
		int eventNoReplyCount = Integer.parseInt(eventStats.get("noreplyCount")
				.toString());
		...
		String query = String
				.format("WITH n(eventid,name,venuename,the_geom,origintype,creatorid,startdate,enddate,category,public,description,attendingcount,declinedcount,maybecount,noreplycount,eventphoto)"
						...
						eventId, eventName, venueName, geomPoint,
						eventStarttimeString, eventEndtimeString,
						eventCategory, eventDescription, eventAttendingCount,
						eventDeclinedCount, eventMaybeCount, eventNoReplyCount,
						eventProfilePicture);
