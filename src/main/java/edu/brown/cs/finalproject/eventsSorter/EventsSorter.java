...
public class EventsSorter {
   ...
   * @return
   */
  public List<Event> sortEventsByAttendingCount(
      List<Event> unsortedEventsList) {

    ResultsCollection resultsCollection = new ResultsCollection();

    for (Event tempEvent : unsortedEventsList) {
      PQNode tempPQNode = new PQNode(
          new Priority(tempEvent.getAttendingCount()), tempEvent);
   ...
   * @return double, the square of the Euclidean distance
   *         between the two points
   */
  private double findDistance(double firstPointLat, double firstPointLng,
      double secondPointLat, double secondPointLng) {
