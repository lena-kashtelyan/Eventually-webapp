    ...
    if (authString != null) {
      ...
      if (auth.verifyAuthToken(username, authToken)) {
        BrowseResultsHolder browseResults = DatabaseManager
            .getUpcomingEventsWithinProximitySortedByPopularity(41.826144690402,
                -71.403125740801, 10000, 100, username);
        ...
        List<Event> suggested = DatabaseManager.getSuggestedEvents(username,
            41.8, -71.4);

        for (Event event : upcoming) {
          String eventID = event.getID();
          userSavedEvents.put(eventID,
              DatabaseManager.checkInterested(eventID, username));
          userAttendingEvents.put(eventID,
              DatabaseManager.checkAttending(eventID, username));
        }
